...
void KEY_Init(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA,ENABLE);                      //使能 PORTA 时钟 
	...
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_2|GPIO_Pin_5; //PA5
...
u8 click_N_Double (u8 time)
{
	  ...
	  if(KEY5==0)  Forever_count++;   //长按标志位未置1
     else        Forever_count=0;
		if(0==KEY5&&0==flag_key)		flag_key=1;	
	  if(0==count_key)
		{
				if(flag_key==1) 
				{
		...
		if(1==KEY5)			flag_key=0,count_key=0;
